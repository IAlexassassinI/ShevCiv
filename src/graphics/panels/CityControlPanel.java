...
public class CityControlPanel extends Panel implements ComponentListener {
    ...
    public void init() {
        ...
        LinkedList<Object> objectsCanBeBuild = getObjectsCanBeBuild();
        Panel objectsToMakePanel = new Panel(Orientation.VERTICAL,0, 0, 660, objectsCanBeBuild.size() * 60, objectsCanBeBuild.size() * 60);
        for(Object object : objectsCanBeBuild) {
            Panel panel = new Panel(0, 0, 660, 60, 660);
            panel.add(new BuildingUnitPanel(object, 0, 0, 600, 60, 600), 600);
    ...
    public class BuildingUnitPanel extends Panel {
        ...
        @Override
        public void render(GameContainer container, Graphics g) throws SlickException {
            if(object instanceof UnitPattern) {
                ...
                g.drawString("Production cost: " + ((Building)object).productionCost, this.x, this.y+20 );
                g.setColor(new Color(0.588f, 0.294f, 0, 1));
                g.drawString(String.valueOf(((Building)object).passiveWealth.production), this.x, this.y+40);
                g.setColor(Color.red);
                g.drawString(String.valueOf(((Building)object).passiveWealth.food), this.x+100, this.y+40);
                g.setColor(Color.orange);
                g.drawString(String.valueOf(((Building)object).passiveWealth.money), this.x+200, this.y+40);
                g.setColor(Color.magenta);
                g.drawString(String.valueOf(((Building)object).passiveWealth.arcanumScience), this.x+300, this.y+40);
                g.setColor(Color.cyan);
                g.drawString(String.valueOf(((Building)object).passiveWealth.engineeringScience), this.x+400, this.y+40);
                g.setColor(Color.white);
                g.drawString(String.valueOf(((Building)object).passiveWealth.societyScience), this.x+500, this.y+40);
            }
        }
    }
