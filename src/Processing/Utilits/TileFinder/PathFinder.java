...
public class PathFinder implements Serializable {
    ...
    static private double maxAvailableActionPoints;
    public static HashMap<Tile,Path> findMovePath(double maxAvailableActionPoints, Tile startTile){
        PathFinder.maxAvailableActionPoints = maxAvailableActionPoints;
        Path FirstPath = new Path(0);
        AllPath = new HashMap<>();
        ...
        for(int i = 0; i < Point.ALL_SIDES.length-1; i++){
            ...
            if(TMP_Tile != null){
                ...
                if(WhereCanBe.FullCheck(TMP_Tile, CurrentUnit.typeOfUnit.whereCanMove)){
                    if(path.currentActionPoints < maxAvailableActionPoints){
                        double modifiers;
                        if((i & 1) == 0){
                        ...
                        if(path.currentActionPoints + actionCost*modifiers <= maxAvailableActionPoints){ //hard variant
    ...
    private static void CaseTree(Path path, Tile CurrentTile, int direction, double ActionCost){
        double UsedActionPoints = path.currentActionPoints + ActionCost;

        if(CurrentTile.isRiver(direction) && !CurrentTile.isBridge(direction)){
            UsedActionPoints = 2147483647; //max int
        }

        if(AllPath.containsKey(CurrentTile)){
            if(AllPath.get(CurrentTile).currentActionPoints <= UsedActionPoints){
                return;
            }
        }
        Path NewPath = new Path(path, CurrentTile, UsedActionPoints);
